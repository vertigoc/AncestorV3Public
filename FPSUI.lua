...
if game.CoreGui:FindFirstChild(tostring(LibName))then 
    game.CoreGui:FindFirstChild(tostring(LibName)):remove();
end;
...
function Kavo.CreateLib(kavName, themeList)
    ...
    function Tabs:NewTab(tabName)
        ...
        coroutine.wrap(function()
            ...
            function Elements:NewTextBox(tname, tTip, callback,placeholder)
            ...
            function Elements:NewSlider(slidInf, slidTip, maxvalue, minvalue,default, callback)
                ...
                if default then 
                    Value=default
                    val.Text=Value;
                    sliderDrag:TweenSize(UDim2.new((Value - minvalue) / (maxvalue - minvalue), 0, 0, 6), "InOut", "Linear", 0.05, true)
                end;   
            end
            ...
            function Elements:NewKeybind(keytext, keyinf, first, callback)
                ...
                game:GetService("UserInputService").InputBegan:connect(function(current, ok) 
                    if current.KeyCode.Name == oldKey and enabled and not ok then 
                        callback(current.KeyCode.Name)
                    end
                end)
...
game.CoreGui.ChildRemoved:Connect(function(gui)
    if tostring(gui)==tostring(LibName)then 
        enabled=false;
    end;
end)
return Kavo
