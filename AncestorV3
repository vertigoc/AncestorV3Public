...
local lib=loadstring(game:HttpGet('https://ancestordevelopment.wtf/ui'))();
local requirements={
    ConfigSetup={
        AlwaysDay=false, 
        AlwaysNight=false,
...
functions.updateChangelog('1st October 2021','Farewell - Ancestor is discontinued for good');
...
--> Gui is parented to playergui for Controller support testing.Caused some bug with UI
functions.FixUILib=function()
    repeat wait()until game:GetService('PlayerGui'):FindFirstChild'Ancestor'
    for i,Child in next,game:GetService('PlayerGui').Ancestor.MainBody.TopBar.LogoTextFolder:GetChildren()do 
        if i==2 then 
            Child.Text='';
        end;
    end;
...
functions.donateBaseAutoload=function(slotnum)
    ...
    if not plot then functions.notify('Ancestor','No Supported Plots Located')requirements.Booleans.DonatingBase=false;return end
...
--> Nobody makes their fucking fly. Deal with it
...
MainPlayerOptions:KeyBind(
    'Fling',
    function(key)
        requirements.ConfigSetup.FlingKey=tostring(key.Name)
        if not requirements.Booleans.Flinging then
            requirements.Booleans.Flinging=true;
            requirements.Booleans.Noclip=true;
            if not requirements.LP.Character.Head:FindFirstChildOfClass('BodyAngularVelocity')then
                local fling = Instance.new('BodyAngularVelocity',requirements.LP.Character.Head)
                fling.AngularVelocity = Vector3.new(0,200000,0)
                fling.MaxTorque = Vector3.new(0,200000,0)
                fling.P = math.huge
            end
        elseif requirements.Booleans.Flinging then	
            requirements.Booleans.Flinging=false;
            requirements.Booleans.Noclip=false;
    ...
    Enum.KeyCode[requirements.ConfigSetup.FlingKey]
);
...
requirements.services.userInputService.InputEnded:Connect(function(key,processed)
    if key.KeyCode==Enum.KeyCode[requirements.ConfigSetup.SprintKey]and not processed then 
        if requirements.Strings.WalkSpeed>16 then 
            ...
            requirements.Strings.WalkSpeed=tonumber(game:GetService('CoreGui').Ancestor.MainBody.ScrollingFrame.WalkSpeedSlider.SliderDarkOutline.SliderValue.Text)
        end;
    end;
end);
...
MainGameOptions:Toggle(
    'Loop Destroy Store Items',
    function(state)
        requirements.Strings.DestroyStores=state;
        if state then 
            functions.destroyStoreItems();
            while wait(7)do
                if not requirements.Strings.DestroyStores then
                    break;
                end;
...
MainGameOptions:Button(
    ...
    function(state)
        pcall(function()
            --LogicStore
            game:GetService('ReplicatedStorage').Interaction.ClientIsDragging:FireServer(workspace.Stores.LogicStore.LDoor);
            game:GetService('ReplicatedStorage').Interaction.ClientIsDragging:FireServer(workspace.Stores.LogicStore.RDoor);
            game:GetService('ReplicatedStorage').Interaction.DestroyStructure:FireServer(workspace.Stores.LogicStore.LDoor);
            game:GetService('ReplicatedStorage').Interaction.DestroyStructure:FireServer(workspace.Stores.LogicStore.RDoor);
            --CarStore
            game:GetService('ReplicatedStorage').Interaction.ClientIsDragging:FireServer(workspace.Stores.CarStore.LDoor);
            game:GetService('ReplicatedStorage').Interaction.ClientIsDragging:FireServer(workspace.Stores.CarStore.RDoor);
            game:GetService('ReplicatedStorage').Interaction.DestroyStructure:FireServer(workspace.Stores.CarStore.LDoor);
            game:GetService('ReplicatedStorage').Interaction.DestroyStructure:FireServer(workspace.Stores.CarStore.RDoor);
            --FurnitureStore
            game:GetService('ReplicatedStorage').Interaction.ClientIsDragging:FireServer(workspace.Stores.FurnitureStore.LDoor);
            game:GetService('ReplicatedStorage').Interaction.ClientIsDragging:FireServer(workspace.Stores.FurnitureStore.RDoor);
            game:GetService('ReplicatedStorage').Interaction.DestroyStructure:FireServer(workspace.Stores.FurnitureStore.LDoor);
            game:GetService('ReplicatedStorage').Interaction.DestroyStructure:FireServer(workspace.Stores.FurnitureStore.RDoor);
        end);
    end,
    true
);
...
local MainGameOptions=MainGameOptionsFrame:Section('Pressure Plate Options');
MainGameOptions:Button(
    ...
    function()
        if syn then 
            ...
            repeat wait()until game:IsLoaded();
                wait(5)
                loadstring(game:HttpGet'https://raw.githubusercontent.com/DarkXRBLX/LT2/main/Ancestor')('We Return ;)');
            ]]);
...
settings:TextLabel('Tested/Moral Support By Adam');
if functions.isPlayerAdmin(requirements.LP)then 
    local adminFunctions={};
    adminFunctions.burnAllItems=function(player)
        -->Removed to stop abuse
    end;
